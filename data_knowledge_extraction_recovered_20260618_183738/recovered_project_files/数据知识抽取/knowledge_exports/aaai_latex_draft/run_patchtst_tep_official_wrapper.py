...
VERSION = "patchtst-official-source-tep-wrapper-v1"
CHECKED_ON = "2026-06-21"
PROJECT_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_CHECKOUT = Path.home() / "aaai_external_baseline_checkouts" / "patchtst"
DEFAULT_OUTPUT = (
    PROJECT_ROOT
    / "knowledge_exports"
...
def _load_official_patchtst_model(checkout: Path):
    supervised_root = Path(checkout).resolve() / "PatchTST_supervised"
    if not (supervised_root / "models" / "PatchTST.py").exists():
        raise FileNotFoundError(f"Official PatchTST supervised source not found: {supervised_root}")
    supervised_text = str(supervised_root)
    if supervised_text not in sys.path:
        sys.path.insert(0, supervised_text)
...
def build_patchtst_config(
    *,
    ...
    if int(patch_len) < 2 or int(patch_len) > int(window_size):
        raise ValueError("patch_len must be in [2, window_size]")
    if int(stride) < 1:
        raise ValueError("stride must be positive")
    if int(d_model) % int(n_heads) != 0:
        raise ValueError("d_model must be divisible by n_heads for the official PatchTST encoder")
...
class OfficialPatchTSTClassifier(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.backbone = official_model_cls(config)
        self.classifier = nn.Sequential(
            nn.LayerNorm(int(config.enc_in)),
            nn.Dropout(float(classifier_dropout)),
            nn.Linear(int(config.enc_in), int(n_classes)),
        )
        self.model_family = "official_patchtst_supervised_backbone_plus_lightweight_classifier"

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        forecast = self.backbone(x)
        if forecast.ndim != 3:
            raise RuntimeError("Official PatchTST backbone must return [batch, pred_len, channel]")
        return self.classifier(forecast[:, -1, :])
...
def train_official_patchtst_classifier(
    official_model_cls,
    *,
    x_train: np.ndarray,
    y_train: np.ndarray,
    ...
    model = OfficialPatchTSTClassifier(
        official_model_cls,
        config=config,
...
def run_seed(
    *,
    ...
    config = build_patchtst_config(
        n_features=int(split.x_train.shape[2]),
        window_size=int(window_size),
        patch_len=int(patch_len),
        stride=int(stride),
        d_model=int(d_model),
    ...
    official_model_cls = _load_official_patchtst_model(checkout)
    train_result = train_official_patchtst_classifier(
        official_model_cls,
    ...
    return {
        ...
        "protocol_alignment": {
            ...
            "official_native_task": "long-term forecasting and representation learning",
            "task_specific_head_added": True,
            "validation_only_model_selection": True,
            "test_labels_used_for_model_selection": False,
            "claim_boundary": "Official PatchTST supervised source backbone adapted with a lightweight TEP classification head; not an official PatchTST paper benchmark score.",
        },
    }
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Run official-source PatchTST supervised backbone with a lightweight TEP classification head.")
    ...
    payload = run_wrapper(
        ...
        revin=not bool(args.disable_revin),
