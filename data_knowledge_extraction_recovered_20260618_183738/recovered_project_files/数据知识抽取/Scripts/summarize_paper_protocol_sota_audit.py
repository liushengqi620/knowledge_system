...
def render_audit() -> str:
    lines: list[str] = []
    lines.append("# Paper Protocol and SOTA Claim Audit")
    lines.append("")
    lines.append(
        "This audit separates method evidence from official SOTA claims. The paper should emphasize "
        "reliability-calibrated mechanism evidence fusion under non-interventional industrial logs, not true "
        "causal graph discovery and not universal official SOTA across every benchmark."
    )
    lines.append("")
    lines.append("## Dataset Claim Matrix")
    lines.append("")
    lines.append("| Dataset | Current protocol evidence | Safe claim | Missing gate before official SOTA |")
    lines.append("|---|---|---|---|")
    lines.append(
    ...
    lines.append("")
    lines.append("## Verified External Source Snapshot")
    lines.append("")
    lines.append("| Source | Verified role | URL or DOI | Paper implication |")
    lines.append("|---|---|---|---|")
    lines.append(
    ...
    lines.append("")
    lines.append("## Baseline Alignment Decision Matrix")
    lines.append("")
    lines.append("| Family | Verified source identity | Current local use | Required before official comparison |")
    lines.append("|---|---|---|---|")
    lines.append(
    ...
    lines.append("")
    lines.append("| Scope | Safe wording | Unsafe wording |")
    lines.append("|---|---|---|")
    lines.append(
    ...
    lines.append("")
    lines.append("## Reproduction Gate Checklist")
    lines.append("")
    lines.append(
        "Before any official SOTA wording is used for a dataset, the cited comparison must match the following gates:"
    )
    lines.append("")
    lines.append("| Gate | Required evidence | Why it matters |")
    lines.append("|---|---|---|")
    lines.append("| split | identical train/validation/test split or a documented reproduced split | random point splits and run-level splits can change fault leakage |")
    lines.append("| preprocessing | identical normalization, variable selection, missing-value handling, and windowing | window and scaling choices can dominate industrial time-series results |")
    lines.append("| metric definition | same Target-F1, Macro-F1, FAR/MAR, event recall, RMSE/MAE, or NASA RUL score | point, event, target, sample, and prognostics metrics are not interchangeable |")
    lines.append("| threshold tuning | same validation-only threshold policy and no test leakage | anomaly detection scores can change dramatically with threshold tuning |")
    lines.append("| delay handling | same fault delay, warm-up, transient removal, and label taxonomy | TEP and process data are sensitive to delay and taxonomy choices |")
    lines.append("| model budget | comparable training rows, epochs, seeds, and hyperparameter search budget | unfair budgets can create artificial SOTA claims |")
    lines.append("| source availability | command, config, seed, and result artifact are archived | AAAI-level claims should be reproducible from artifacts |")
    lines.append("")
    lines.append("## Dataset-Specific Next Actions")
    lines.append("")
    lines.append("| Dataset | Highest-value next action | Expected paper impact |")
    lines.append("|---|---|---|")
    lines.append("| TEP | Reproduce two cited external TEP papers under their exact split/preprocessing/taxonomy/metric or explicitly mark them as non-identical references | upgrades TEP from strong matched-protocol evidence toward defensible SOTA wording |")
    lines.append("| SKAB | Align USAD/TranAD/GDN/MTAD-GAT with official repositories and freeze threshold/event-scoring policy | decides whether SKAB can move beyond method evidence |")
    lines.append("| Hydraulic | Reformat results to match the published four-target table and add one temporal neural baseline | strengthens safety benchmark credibility |")
    lines.append("| C-MAPSS | Add or cite compatible published RUL baselines under the same terminal-test protocol and keep the pseudo-terminal/PHM-score trade-off note in the result table | strengthens original-task transfer story, but not required for the main TEP-centered paper |")
    lines.append("")
    lines.append("## Paper Integration")
    lines.append("")
    lines.append(
        "Use this audit after `paper_final_tables.md` in the experiments and limitations narrative. Table 1 can "
        "present the positive multi-benchmark results, while this audit states the exact claim boundary. The "
        "recommended final manuscript stance is: strong method evidence with TEP as the primary positive "
        "matched-protocol benchmark, SKAB as reliability-editing evidence, and Hydraulic/C-MAPSS as safety "
        "and RUL-transfer evidence."
    )
    lines.append("")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Render protocol and SOTA claim audit for the paper package.")
